#ifndef CLOUDS_RAY_MARCHING
#define CLOUDS_RAY_MARCHING
...
v2f vert(appdata v)
{
	...
	o.screenpos = TransformTriangleVertexToUV(o.vertex.xy);
...
float noise(float t) { return SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, float2(t, .0) / float2(_NoiseTexSize, _NoiseTexSize)).x; }
float moy = 0.;
...
float noise(float3 p)
{
	...
	float2 rz = SAMPLE_TEXTURE2D(_NoiseTex, sampler_NoiseTex, (tap + 0.5) / 256.0).yx;
...
float4 mainImage(float2 q_input)
{
	/*float2 q = fragCoord.xy / _ScreenParams.xy;*/
	//float2 q = fragCoord.xy;

	float2 q = float2(q_input.x, 1 - q_input.y);
	float2 p = q - 0.5;
